...
(function(){
  ...
  document.querySelectorAll('.blog-item').forEach(item => {
    ...
    btn.addEventListener('click', () => {
      ...
      if(!isOpen){
        ...
        btn.innerHTML = 'Read Less <span class="plus">+</span>';
      } else {
        item.classList.remove('open');
        full.style.maxHeight = null;
        btn.innerHTML = 'Read More <span class="plus">+</span>';
      }
    });
  });
      ...
      chip.addEventListener('click', () => {
        ...
        if(visibleCount === 0){
          if(!emptyMsg){
            ...
            emptyMsg.textContent = 'No posts in this category yet — check back soon.';
  ...
  if(writeReviewBtn){
    writeReviewBtn.addEventListener('click', () => {
      showToast('Review form coming soon — message us on WhatsApp for now');
    });
  }
